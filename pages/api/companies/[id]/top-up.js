...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    ...
    try {
      ...
      const result = await prisma.$transaction(async (prisma) => {
        ...
        const pointTransfer = await prisma.pointTransfer.create({
          data: {
            ...
            description: 'пополнение',  // Описание операции
          },
        });
      ...
      res.status(200).json({
        type: 'success',  // Добавляем тип "success"
        message: `Баланс сотрудника пополнен на ${points} баллов. Баланс компании уменьшен на ${points} баллов.`,
        data: result,  // Возвращаем результат операции (если нужно для дальнейшей обработки на фронте)
      });
